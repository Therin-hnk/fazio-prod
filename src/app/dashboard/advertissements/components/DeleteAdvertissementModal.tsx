// app/admin/advertissements/components/DeleteAdvertissementModal.tsx
// Modal pour confirmer la suppression d'une ou plusieurs publicités.
...
interface DeleteAdvertissementModalProps {
  advertissement: Advertissement | null;
  selectedAdvertissementCount?: number;
  onConfirm: () => Promise<void>;
  onCancel: () => void;
}

function DeleteAdvertissementModal({ advertissement, selectedAdvertissementCount = 0, onConfirm, onCancel }: DeleteAdvertissementModalProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  if (!advertissement && selectedAdvertissementCount === 0) return null;
  ...
  const isMultipleAdvertissements = !advertissement?.id || selectedAdvertissementCount > 1;
        ...
        <div className="p-6">
          ...
          <div className="text-center mb-6">
            ...
            <p id="delete-modal-description" className="text-gray-600 text-sm sm:text-base leading-relaxed">
              {isMultipleAdvertissements ? (
                <>
                  Voulez-vous vraiment supprimer{' '}
                  <span className="font-semibold text-gray-900">{selectedAdvertissementCount || 'les'}</span> publicité(s)
                  sélectionnée(s) ?
                </>
              ) : (
                <>
                  {"Voulez-vous vraiment supprimer la publicité"}{' '}
                  <span className="font-semibold text-gray-900">{advertissement?.name}</span> ?
                </>
              )}
            </p>
...
export default memo(DeleteAdvertissementModal);