...
interface CategoryFilterProps {
  onFilterChange: (eventId: string | null, search: string) => void;
}
...
function CategoryFilter({ onFilterChange }: CategoryFilterProps) {
  ...
  const debouncedFilterChange = useCallback(
    debounce((eventId: string | null, search: string) => {
      onFilterChange(eventId, search);
    }, 300),
    [onFilterChange]
  );
  ...
  const handleEventChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    debouncedFilterChange(eventId, search);
  };

  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const searchValue = e.target.value;
    setSearch(searchValue);
    debouncedFilterChange(selectedEventId, searchValue);
  };
