// app/admin/organizations/components/OrganizationFilter.tsx
// Composant pour filtrer les organisations, similaire à EventFilter.
...
interface OrganizationFilterProps {
  onFilterChange: (search: string, sex: string | null) => void;
}
...
function OrganizationFilter({ onFilterChange }: OrganizationFilterProps) {
  ...
  const debouncedOnFilterChange = useCallback(
    debounce((searchTerm: string, sex: string | null) => {
      onFilterChange(searchTerm, sex);
    }, 300),
    [onFilterChange]
  );

  useEffect(() => {
    debouncedOnFilterChange(search, selectedSex);
  }, [search, selectedSex, debouncedOnFilterChange]);

  const handleSexChange = (value: string) => {
    setSelectedSex(value ? value : null);
  };
  ...
  const hasActiveFilters = selectedSex !== null || search !== '';
  const activeFiltersCount = (selectedSex ? 1 : 0) + (search ? 1 : 0);
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-3">
              <label htmlFor="sexFilter" className="block text-sm font-semibold text-gray-700">
                Filtrer par sexe
              </label>
