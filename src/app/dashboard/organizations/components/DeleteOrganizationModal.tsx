// app/admin/organizations/components/DeleteOrganizationModal.tsx
// Modal pour confirmer la suppression d'une ou plusieurs organisations, similaire à DeleteEventModal.
...
function DeleteOrganizationModal({ organization, selectedOrganizationCount = 0, onConfirm, onCancel }: DeleteOrganizationModalProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  if (!organization && selectedOrganizationCount === 0) return null;
  ...
  const isMultipleOrganizations = !organization?.id || selectedOrganizationCount > 1;
            ...
            <p id="delete-modal-description" className="text-gray-600 text-sm sm:text-base leading-relaxed">
              {isMultipleOrganizations ? (
                <>
                  ...
                  <span className="font-semibold text-gray-900">{selectedOrganizationCount || 'les'}</span> organisation(s)
                  sélectionnée(s) ?
                </>
              ) : (
                <>
                  {"Voulez-vous vraiment supprimer l'organisation"}{' '}
                  <span className="font-semibold text-gray-900">{organization?.firstName} {organization?.lastName}</span> ?
                </>
              )}
            </p>
