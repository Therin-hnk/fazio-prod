          ...
          <div className="hidden lg:flex items-center space-x-1 xl:space-x-2">
            {menuItems.map((item) => (
              ...
              >
                ...
                <span className={item.active ? 'border-b-2 border-orange-500' : ''}>
                  {item.label}
                </span>
              </a>
            ))}
          ...
          <div className="hidden md:flex lg:hidden items-center space-x-1">
            {menuItems.map((item) => (
              <a
                ...
                title={item.label}
              >
                <item.icon className={`w-4 h-4 ${item.active ? 'border-b-2 border-orange-500' : ''}`} />
              </a>
            ))}
          </div>
        ...
        <div className="px-2 pt-2 pb-4 bg-white border-t border-gray-200">
          {menuItems.map((item) => (
            ...
            >
              ...
              <span className={item.active ? 'border-b border-orange-500' : ''}>
                {item.label}
              </span>
            </a>
          ))}
